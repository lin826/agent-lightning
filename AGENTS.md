...
- Each training variant MUST have its own BM25 retrieval server (`serve/serve_retrieval_<variant>.bsub` → `outputs/bm25_server_addr_<variant>.txt`), and full-test eval MUST use a separate eval server (`serve/serve_retrieval_eval_<variant>.bsub` → `outputs/bm25_server_addr_eval_<variant>.txt`). Sharing a server across variants or between train and eval causes cross-talk in retrieval load and addr-file races. See the pairing table in `contrib/recipes/search_r1/README.md`. GEPA full-test eval triggers in `scripts/gepa_full_eval.py` auto-submit `serve/serve_retrieval_eval_gepa.bsub` when no healthy eval server is running.
