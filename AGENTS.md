...
- Each training variant MUST have its own BM25 retrieval server (`serve/serve_retrieval_<variant>.bsub` → `outputs/bm25_server_addr_<variant>.txt`). Sharing a server across variants causes cross-talk in retrieval load and addr-file races. See the pairing table in `contrib/recipes/search_r1/README.md`.
